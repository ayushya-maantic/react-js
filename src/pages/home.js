...
function Add() {
  ...
  const handleSubmit = (event) => {
    ...
    if (valid && url_reg.test(inputs.AppBaseLink)) {
      axios.post(`${API_HOST}/update`, inputs, { 'headers': { 'Content-Type': 'application/json' } })
  ...
  const fetchInventory = (num) => {
    ...
    axios.get(`${API_HOST}/${num}?${urlParameters}`)
      .then(res => setItems(eval(res.data.r)))
  ...
  const fetchCount = () => {
    var urlParameters = Object.entries(showParams).map(e => e.join('=')).join('&');
    axios.get(`${API_HOST}/count?${urlParameters}`)
      .then(res => {
